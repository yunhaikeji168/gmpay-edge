...
- 编辑 root 用户的资料或凭据必须由当前启用的 root 用户执行；身份检查与用户、凭据和
  Session 写入处于同一批次，并发分配 root 角色不能绕过检查。`settings:read` 仍是能读取
  运行时密钥的高敏权限，不应作为普通监控权限授予。
- 付款审核批准、支付入账、批准审计和通知 outbox 一起提交。并发拒绝会使审批产生的支付
  写入全部回滚；验证或持久化失败时审核保持待处理，可安全重试。
- 管理员取消／退款将订单、审计和持久化 outbox 原子提交；提交后的 Queue 派发失败可由
  outbox 恢复。
- 支付扫描使用订单快照的资产和目标，缓存键包含收款方式、资产和目标；连接故障切换
  不会切换资产。
- Provider-event 迁移 `0007_sparkling_wallflower.sql` 新增可空租约令牌，不重写已有行。
  单次租约为五分钟，覆盖最多八次各 30 秒的 EVM 查询及额外处理时间。入账、完成、失败
  和连接健康更新均检查令牌及到期时间；恢复操作会使旧持有者失效。Worker 部署前运行正常
  D1 迁移，Bun 在启动时迁移；旧的处理中记录在原租约到期后仍可恢复。
